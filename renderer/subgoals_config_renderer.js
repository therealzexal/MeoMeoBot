...
function getPreviewHtml() {
    ...
    const css = `
        ...
        body { margin: 0; padding: 0; overflow: hidden; font-family: 'Inter', sans-serif; background: transparent; display: flex; justify-content: center; align-items: center; height: 100vh; }
        .progress-container { position: relative; width: 90%; max-width: 800px; height: 40px; background: rgba(0, 0, 0, 0.5); border-radius: 20px; border: 2px solid rgba(255, 255, 255, 0.2); }
        .markers-container { position: absolute; top: 0; left: 0; width: 100%; height: 100%; border-radius: 18px; overflow: hidden; pointer-events: none; z-index: 5; }
        ...
        .step-marker { position: absolute; top: 0; bottom: 0; width: 2px; background: rgba(255, 255, 255, 0.8); pointer-events: none; transform: translateX(-50%); }
        .step-label { position: absolute; top: -45px; background: rgba(0, 0, 0, 0.8); color: #fff; padding: 6px 10px; border-radius: 6px; font-size: 0.9rem; white-space: nowrap; border: 1px solid rgba(255, 255, 255, 0.3); z-index: 6; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
        .step-label::after { content: ''; position: absolute; bottom: -6px; left: var(--arrow-pos, 50%); transform: translateX(-50%); width: 0; height: 0; border-left: 6px solid transparent; border-right: 6px solid transparent; border-top: 6px solid rgba(0, 0, 0, 0.8); }
    ...
    if (steps && Array.isArray(steps)) {
        ...
        sortedSteps.forEach(step => {
            ...
            if (!isNaN(stepRelative)) {
                ...
                markersHtml += `
                    <div class="step-marker" style="left: ${stepPercentage}%; transform: translateX(-50%)"></div>
                `;
                ...
                if (isNextGoal) {
                    ...
                    labelsHtml += `
                        <div class="step-label" style="left: ${stepPercentage}%; transform: translateX(-${shift}%); --arrow-pos: ${shift}%">
                            ${step.label}
                        </div>
                    `;
                }
